...
def validate_sql(sql: str, semantic_model: dict[str, Any] | None = None) -> str:
    ...
    disallowed = found_tables - _allowed_tables(active_model) - NON_TABLE_IDENTIFIERS
    if disallowed:
...
async def generate_sql(
    ...
    *,
    ...
    active_model = semantic_model or load_semantic_model('')
    model_for_prompt = {
        'tables': active_model.get('tables', {}),
        'dimensions': _normalize_dimensions(active_model),
        'metrics': _normalize_metrics(active_model),
    }
    model_yaml = yaml.dump(model_for_prompt, default_flow_style=False, width=120)
    prompt = SQL_AGENT_PROMPT.format(
        schema=model_yaml,
        question=question,
        allowed_tables=', '.join(sorted(_allowed_tables(active_model))),
    ...
    try:
        ...
        async with analytics_session() as a_db:
            ...
            try:
                ...
                fixed_validated = validate_sql(fixed_sql, semantic_model=semantic_model)
                safe_sql, params = prepare_query(
                    fixed_validated,
                    auth,
                    app_id,

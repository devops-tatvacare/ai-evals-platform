"""ApplicationSetting model - user/app configuration."""
...
class ApplicationSetting(Base, TenantUserMixin, ShareableMixin):
    ...
    forked_from: Mapped[int | None] = shareable_int_forked_from("application_settings")
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    )

    __table_args__ = (
        Index("idx_application_settings_tenant", "tenant_id"),
        Index("idx_application_settings_tenant_user", "tenant_id", "user_id"),
        UniqueConstraint("tenant_id", "app_id", "key", "user_id", "visibility", name="uq_application_setting"),
        Index(

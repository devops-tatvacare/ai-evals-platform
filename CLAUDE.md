...
- **Verify table shape and runtime gates against the real source — never assume.** Before writing raw SQL against a table, read the ORM model file (`backend/app/models/<table>.py`) AND run `\d schema.table` against the live DB (docker-compose Postgres on `localhost:5433`). Before gating runtime behavior on a config flag, trace the gate read at the actual consumer — don't assume a flag with a related name is the right one. **`unittest.mock` / FakeSession tests are not a substitute** — fakes paper over shape mismatches that only show up against a real DB. Two same-day bugs on 2026-05-18, both shipped by the same author against unit tests that passed: (1) Alembic 0052 assumed `platform.application_settings.id` was uuid (it's `integer DEFAULT nextval`), assumed a `created_at` column existed (only `updated_at` does), and used `'shared'` for visibility (DB stores `'SHARED'` because the `SAEnum` is `native_enum=False` → member NAME) — caught by docker boot CrashLoopBackoff; (2) `config_validator.py` check 8 gated the narrative-cascade check on `app.config.analytics.singleRun.aiSummary.enabled` but the actual production gate is `report_config.narrative_config.enabled` (per-ReportConfiguration column) — latent silent narrative-degradation, never crashes. Both required `docker compose run --rm migrate` and a backend boot against live DB to catch. Run those before claiming "done".
- **No needless multi-line comments.** One-line comments only — only when the WHY is non-obvious (hidden constraint, workaround for a specific bug, behavior that would surprise a reader). Never restate WHAT the code does; well-named identifiers handle that. No multi-paragraph block docstrings on internal functions, no "Phase X investigation" prose, no decision logs, no "removed in commit Y" markers — those belong in the PR description, the commit message, or `docs/plans/`. Module-level docstrings stay short — one sentence on what the module is, not a phase-history changelog. If a comment is longer than the code it describes, delete it.
